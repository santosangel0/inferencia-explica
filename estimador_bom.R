...
# =====================================================
# Estimador Bom: proporção amostral p_hat = x/n
# =====================================================
...
estimador_bom <- function(amostra) mean(amostra)
...
resultados <- do.call(rbind, lapply(n_vals, function(n) {
  do.call(rbind, lapply(seq_len(n_sim), function(sim) {
    ...
    estimativa <- estimador_bom(amostra)
...
ggplot() +
  geom_line(
    ...
    color = "#4A90D9", alpha = 0.35, linewidth = 0.6
  ) +
  geom_point(
    data  = resultados,
    aes(x = n, y = estimativa),
    color = "#4A90D9", alpha = 0.35, size = 1.2
  ) +
  geom_line(
    data      = medias,
    aes(x = n, y = media),
  ...
  labs(
    title    = "Estimador Bom — Proporção Amostral (p̂ = x/n)",
...
ggsave("graficos/grafico_estimador_bom.png", width = 10, height = 6, dpi = 150)
message("Salvo: graficos/grafico_estimador_bom.png")
